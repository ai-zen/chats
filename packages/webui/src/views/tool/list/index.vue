<template>
  <div class="tool-list-page">
    <el-page-header content="工具列表" @back="$router.back()">
      <template #extra>
        <el-button plain type="success" @click="create">
          <el-icon><Plus /></el-icon>&ensp; 新增工具
        </el-button>
      </template>
    </el-page-header>
...
async function getList() {
  try {
    ...
    listState.list = await api.getToolList();
...
function create() {
  router.push({ path: "/tool/form", query: { mode: FormMode.Create } });
}

function edit(id: string) {
  router.push({ path: "/tool/form", query: { mode: FormMode.Edit, id } });
}
