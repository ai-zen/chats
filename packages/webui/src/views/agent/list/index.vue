<template>
  <div class="list-page">
    <el-page-header content="代理列表" @back="$router.back()">
      <template #extra>
        <el-button plain type="success" @click="create">
          <el-icon><Plus /></el-icon>&ensp; 新增代理
        </el-button>
      </template>
    </el-page-header>
...
function create() {
  router.push({ path: "/agent/form", query: { mode: FormMode.Create } });
}

function edit(id: string) {
  router.push({ path: "/agent/form", query: { mode: FormMode.Edit, id } });
}
